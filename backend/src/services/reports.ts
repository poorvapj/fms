...
export async function runReport(key: string, query: Record<string, any>, user: AuthUser): Promise<Report> {
  ...
  switch (key) {
    ...
    case 'open_aging': {
      const out = rows.filter((r) => LIVE_STATUSES.includes(r.status))
        .map((r) => ({ ...listBase(r, names), stage: stageName(r.current_stage_key), age_days: round((diffMinutes(now, r.requested_at) ?? 0) / 1440) }))
        .sort((a, b) => (b.age_days ?? 0) - (a.age_days ?? 0));
      sections.push({
        title: `Open Job Aging (${out.length})`,
        columns: [...LIST_COLUMNS, { key: 'status', label: 'Status', type: 'status' }, { key: 'stage', label: 'Current stage' }, { key: 'age_days', label: 'Age (days)', type: 'number' }],
        rows: out,
      });
      break;
    }
